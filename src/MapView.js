...
const MapComponent = ({
  ...
  // Signature of worker identities so we only recreate markers when the set of workers changes
  const workerIdsSignature = (workers || [])
    .map((w) => w.id)
    .filter(Boolean)
    .sort()
    .join(",");
  ...
  useEffect(() => {
    ...
    // Prepare active workers with a usable coordinate for the map
    const activeWorkersWithCoordinates = workers
      .filter((worker) => worker.isActive !== false)
      .map((worker) => {
        // Prefer explicit numeric coordinates from Firestore
        let lat =
        ...
        console.log("[MapView] Raw worker coordinates", {
          id: worker.id,
          Name: worker.Name || worker.displayName || worker.name,
          City: worker.City || worker.city,
        ...
        const hasValidStoredCoords =
          typeof lat === "number" &&
          typeof lng === "number" &&
          !(lat === 0 && lng === 0);

        // If no valid stored coordinates, fall back to city-based or default coordinates
        if (!hasValidStoredCoords) {
          const cityName = worker.City || worker.city || "";
          const cityKey = cityName.toLowerCase();
          const cityConfig = PAKISTAN_CITIES[cityKey];

          if (cityConfig) {
            lat = cityConfig.lat;
            lng = cityConfig.lng;
          } else {
            // Fallback: place worker near the default Pakistan center
            lat = CONFIG.PAKISTAN_CENTER.lat + (Math.random() - 0.2) * 0.5;
            lng = CONFIG.PAKISTAN_CENTER.lng + (Math.random() - 0.2) * 0.5;
          }
        }

        if (typeof lat !== "number" || typeof lng !== "number") {
          console.warn("[MapView] Skipping worker due to invalid coordinates", {
            id: worker.id,
            lat,
            lng,
          });
        ...
        console.log("[MapView] Using worker coordinates for map", {
          id: enriched.id,
          Name: enriched.Name || enriched.displayName || enriched.name,
          _mapLat: enriched._mapLat,
          _mapLng: enriched._mapLng,
        });
    ...
    if (typeof window !== "undefined") {
      window.__skillzaarMapWorkersDebug = activeWorkersWithCoordinates;
    }
    ...
    const newWorkerMarkers = activeWorkersWithCoordinates.map((worker) => {
      ...
      // Use profile picture as marker icon when available, otherwise a blue dot
      let markerIcon;
      if (profilePic) {
        markerIcon = {
          url: profilePic,
      ...
      console.log("[MapView] Worker marker created", {
        id: worker.id,
        name: workerName,
        position,
      });
  ...
  }, [map, workerIdsSignature, onWorkerClick]);
  ...
  // Allow parent to trigger a preview route between a specific job and worker
  useEffect(() => {
    if (!onPreviewRouteRequest || !map || !window.google || !window.google.maps)
      return;

    onPreviewRouteRequest.current = (jobId, workerId) => {
      try {
        const jobMarker = markersRef.current.find((m) => m.jobId === jobId);
        const workerMarker = workerMarkersRef.current.find(
          (m) => m.workerId === workerId
        );

        if (jobMarker && workerMarker) {
          drawConnectionLine(
            jobMarker.getPosition(),
            workerMarker.getPosition()
          );
        }
      } catch (e) {
        console.warn("[MapView] Failed to preview route", e);
      }
    };
  }, [onPreviewRouteRequest, map, drawConnectionLine]);
...
const MapView = () => {
  ...
  const [selectedJobForAssignment, setSelectedJobForAssignment] =
    useState(null);
  const previewRouteRef = useRef(null);
    ...
    <div className="map-view-container">
      ...
      <div className="map-content">
        <MapComponent
          ...
          onPreviewRouteRequest={previewRouteRef}
        />
      </div>
      ...
      <JobAssignmentModal
        ...
        onPreviewRoute={(jobId, workerId) => {
          if (previewRouteRef.current) {
            previewRouteRef.current(jobId, workerId);
          }
        }}
      />
    </div>
